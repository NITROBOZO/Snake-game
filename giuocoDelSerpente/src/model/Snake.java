...
public class Snake {
	...
	// IMPOSTA LUNGHEZZA E POSIZIONE IN BASE AL PLAYER
	...
	public void move(int c, int fieldSize, int cellSize, boolean m, boolean walls) {
		...
		if ((c == KeyEvent.VK_W && this.lastKey != KeyEvent.VK_S)
				...
				|| (c == KeyEvent.VK_D && this.lastKey != KeyEvent.VK_A) || this.lastKey == 255) {
			this.lastKey = c;
		}
		if (walls) {
			switch (this.lastKey) {
	...
	/*
	 * per colpa di un bug ,nel primo frame il nuovo pezzo veniva messo di lato al
	 * serpente o li vicino
	 */
